    ...
    {loading ? <p className="empty-state">Loading your Project Journey…</p> : error ? <p className="empty-state error-state" role="alert">{error}</p> : sessions.length ? <div className="journey-session-list">
      {sessions.map(item => <article key={item.sessionId} className={`journey-session-card ${item.status}`}>
        ...
        <div className="journey-session-content"><div className="journey-session-heading"><div><small>{new Date(`${item.sessionDate}T00:00:00`).toLocaleDateString()}</small><h3>{item.focus}</h3></div><span className={`activity-control-status ${item.status}`}>{item.status === "open" ? "Current" : item.status === "closed" ? "Closed" : "Upcoming"}</span></div>
          ...
          <div className="journey-track-action">{item.sessionNumber===10 ? item.status==="open"?<button type="button" className="secondary compact" onClick={()=>setFeedbackSession(item)}>{item.platformFeedbackCompletedAt?<><CheckCircle2 size={14}/> Feedback completed</>:"Feedback →"}</button>:item.status==="closed"&&item.platformFeedbackCompletedAt?<button type="button" className="secondary compact" onClick={()=>setFeedbackSession(item)}><CheckCircle2 size={14}/> View feedback</button>:<span>{item.status==="closed"?"Feedback closed":"Feedback opens with S10"}</span> : item.status === "closed" ? item.sessionNumber>=6&&item.sessionNumber<=9&&item.workTrackUpdatedAt?<button type="button" className="secondary compact" onClick={()=>setTrackSession(item)}><CheckCircle2 size={14}/> View track</button>:<span>Track closed</span> : item.status === "open"&&item.sessionNumber>=6&&item.sessionNumber<=9 ? <button type="button" className="secondary compact" onClick={() => setTrackSession(item)}>{item.workTrackUpdatedAt?<><CheckCircle2 size={14}/> Track saved</>:"Track →"}</button> : <span>Track upcoming</span>}</div>
        </div>
      </article>)}
