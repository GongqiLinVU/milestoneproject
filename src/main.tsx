...
function StudioSessionControl({
  ...
  function evidenceState(row:TeacherTrackRow){if(!row.response)return"Missing";const completion=Number(row.response.completionPercent);if(Number.isFinite(completion))return (row.teacherVerifiedCompletion!=null?"Verified ":"")+completion+"% · "+String(row.response.completionStage||completionStage(completion));return String(row.response.overallReadiness||row.response.verificationStatus||row.response.endToEnd||"Tracked");}
