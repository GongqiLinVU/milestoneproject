...
export function RosterManager() {
  ...
  async function saveStudent(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setMessage("");const v=Object.fromEntries(new FormData(event.currentTarget));const payload={block_id:blockId,student_id:String(v.student_id).trim(),full_name:String(v.full_name).trim(),preferred_name:String(v.preferred_name||"").trim()||null,vu_email:String(v.vu_email).trim().toLowerCase(),team_number:Number(v.team_number),project_name:String(v.project_name||"").trim()||null};const{error}=await supabase.from("student_roster").upsert(payload,{onConflict:"block_id,student_id"});setBusy(false);if(error)return setMessage(error.code==="23505"?"That Student ID or email already exists in this block.":"Student could not be saved.");setStudentModal(null);setMessage("Student saved.");await loadRoster();}
  function parseCsv(){const lines=csv.trim().split(/\r?\n/).filter(Boolean),skip=/student.?id/i.test(lines[0]||"")?1:0;setPreview(lines.slice(skip).map((line,index)=>{const[student_id="",full_name="",preferred_name="",vu_email="",team="",project_name=""]=line.split(",").map(cell=>cell.trim().replace(/^"|"$/g,""));const team_number=Number(team.replace(/\D/g,""));let error="";if(!student_id||!full_name||!vu_email)error="Missing required details";else if(!/^\S+@\S+\.\S+$/.test(vu_email))error="Invalid email";else if(team_number<1||team_number>8)error="Team must be 1–8";return{line:index+skip+1,block_id:blockId,student_id,full_name,preferred_name:preferred_name||null,vu_email:vu_email.toLowerCase(),team_number,project_name:project_name||null,error:error||undefined};}));}
  async function confirmImport(){const valid=preview.filter(row=>!row.error).map(({line,error,...row})=>row);if(!valid.length)return;setBusy(true);const{error}=await supabase.from("student_roster").upsert(valid,{onConflict:"block_id,student_id"});setBusy(false);if(error)return setMessage("CSV import failed. Check duplicate emails and roster values.");setImportModal(false);setCsv("");setPreview([]);setMessage(`${valid.length} students imported.`);await loadRoster();}
