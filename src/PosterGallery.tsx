...
export function StudentPosterGallery({ onFeedback }: { onFeedback: (teamName: string) => void }) {
  ...
  return <section className="poster-gallery-student" aria-labelledby="poster-gallery-title">
    ...
    {uploadStatus && <article className="my-poster-panel"><div><span>Your Team Poster · {uploadStatus.teamName}</span><b>{uploadStatus.draft ? uploadStatus.draft.originalFilename : "No Poster uploaded yet"}</b><small>Project Advertisement, not a Mini Report · PDF preferred · PNG/JPG accepted · exactly one page · maximum 1 MB</small></div>{uploadStatus.draft && <div className="my-poster-preview"><PosterPreview versionId={uploadStatus.draft.versionId} mimeType={uploadStatus.draft.mimeType} title={`${uploadStatus.teamName} draft`} compact/></div>}<UploadControl teamId={uploadStatus.teamId} current={uploadStatus.draft} onUploaded={load}/></article>}
